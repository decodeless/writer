...
public:
    ...
    void*                 data() const { return m_linearResource.backing_resource().arena(); }
    size_t size() const { return m_linearResource.backing_resource().bytesAllocated(); }

    template <class T, class... Args>
    T* create(Args&&... args) {
        return decodeless::create<T>(m_linearResource.backing_resource(),
                                     std::forward<Args>(args)...);
    }

    template <class T>
    std::span<T> createArray(size_t size) {
        return decodeless::createArray<T>(m_linearResource.backing_resource(), size);
    }

#ifdef __cpp_lib_ranges
    template <std::ranges::input_range Range>
    std::span<std::ranges::range_value_t<Range>> createArray(const Range& range) {
        return decodeless::createArray(m_linearResource.backing_resource(), range);
    }
#endif
